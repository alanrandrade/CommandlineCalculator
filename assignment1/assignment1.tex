...
%% used in the various phases of the testing process (planning, preparation, test geIn the section \ref{levels} we told about the units our SUT could be split in. We consider every combination of unitneraation, test
%% execution, completion), which tests are performed manualpart that should be tested. Per such a part we will consider ourselves finished if we feel certain that we can predict every outcome for every input. Since the SUT is definedly, whaat is automated, and which
%% tools have to be obtained or developed.                            , this certainty can be achieved. When all parts have been tested in such a way, we consid
rselves finished.
